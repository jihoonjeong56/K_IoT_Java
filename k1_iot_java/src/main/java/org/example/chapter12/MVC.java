...
*   - 상태변경에 대해 View와 Controller 에게 알릴 수 있는 메커니즘을 제공
...
* == MVC 패턴 장점 ==
* - 역할 분담
*   : 각 구성 요소가 독립적인 기능으 책임 > 코드관리 & 확장용이
*
* - 유지보수 용이
...
* == MVC 단점
* - 복잡성 증가
*   : 간단한 애플리케이션 구현하는 MVC 패턴 구조가 복잡성을 증가시킴
* - 학습 곡선 높음
...
*
* cf) Spring: 자바기반 웹 프레임 워크
*   >> Spring Boot(Spring MVC 자동 내장)
*
*
* */
// - 학교 급식 시스템 -
// Mode(모델) : 급식표(데이터) - 급식 메뉴가 적힌 종이
// View(뷰) - 칠판(화명) - 학생들이 확인하는 콧
// Controller(컨트롤러) - 영양사 선생님 - 급식표 내용을 업데이트하고, 칠판에 알려주는 역할

// MVC 동작 흐름(원리)
// 학생(사용자) >>> View >>  영양사 >> 급식표 조회 >> Controller가 View에 전달 >> View가 출력
...
// 3. Controller: Model과 View 를 연결하는 역할
...
public class MVC {
    public static void main(String[] args) {
        ...
        controller.setLunchMenu("깁밥, 떡볶이, 콜라");
       // controller.updateView();
        controller.studentRequestMenu();

        controller.setLunchMenu("김밥, 매우 떡볶이, 콜라");
       // controller.updateView();
        controller.studentRequestMenu();
