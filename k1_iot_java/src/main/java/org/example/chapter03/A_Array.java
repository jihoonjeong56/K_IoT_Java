...
public class A_Array { //=== 클래스명 == 파일명(파일명은 대문자로 시작. + UpperCamelCase 사용)
    ...
    public static void main(String[] args) {
        ...
        // - 데이터를 "연속적"으로 저장
        // - 순서 존재 (index 인덱스 번호 - 0부터 시작하여 1씩 증가되는 값)
...
//        int scores1[];
        ...
        //cf) 배열은 생성 시 모든 요소는 해당 데이터 타입의 기본값으로 초기화
        // >> 숫자(0), 문자열(""), 논리형(false)

        // cf) 참조자료형은 -주소값이 저장
        // : 데이터 배열이 저장된 첫 번째 메모리 주소가 저장(인덱스번소 0번의 주소)
        ...
        scores1[0] = 80;
        scores1[1] = 90;
        scores1[2] = 85; //전체요소의 개수 -1 == 마지막 인덱스 번호

        System.out.println(scores1[0]);//80
        System.out.println(scores1[1]);//90
        System.out.println(scores1[2]);//85
        //System.out.println(scores1[3]); ArrayIndexOutOfBoundsException Error
        //: 배열의 크기값을 초과할 경우 접근 불가 예외발생

        //1) 배열의 선언 : int[] numbers;
        //2) 배열의 생성 : numbers = new int[3];
        //3) 배열의 초기화 : 각각의 위치 인데스 번호를 사용하여 데이터 삽입

        // == 배열의 선언과 동시에 초기화==
        //: new 연산자 사용 + 직접적으로 대입하여 배열 생성 시 길이 지정 하지 않음.
        //>> 요소의 개수를 파악하여 길이가 자동 지정

        // 1) 데이터타임[] 배열명 = new 데이터타입[] {요소1, 요소2, 요소3 ......};
        int[] number1 = new int[]{1, 2, 3, 4, 5};
        System.out.println(number1[0]);
        // 2) 데이터타임[] 배열명 = {요소1, 요소2, 요소.....};
        char[] greet = {'h', 'e', 'l', 'l', 'o'}; //동일한 데이터 중복저장 가능
        ...
        //cf) (변수명 || 데이터타입)에 맞침표. 를 사용한 기능
        //  >> 해당 데이터 타입 내에 내장된 자바 기능 활용
        // == 배열의 특징 ==
        // > 한 번 생성되면 크기 변경 X
        // > 참조 타입 : 주소값이 저장
        // > 모든 요소는 같은 자료형을 가진다.
        ...
        System.out.println(a.toString());//toString() : 문자열로 출력
        ...
        // == 문자열 배열 생성==
        // : String(참조자료형)을 요소값으로 사용
        // >> 초기값 null
        String[] fruits = new String[3];
        System.out.println(fruits); // 주소값 [Ljava.lang.String;@28a418fc
        ...
        for (int index = 0; index < original.length; index++) {
            //배열 요소의 개수(길이)만큼 반복
            //for 문 내에서 반복 중 removeIndex와 index 번호가 일치하는경우
