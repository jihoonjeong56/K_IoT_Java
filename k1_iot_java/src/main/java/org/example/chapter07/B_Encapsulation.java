...
//      >>데이터엥 접근하거나 수정할 때는 getter & setter 메서드를 사용하는 체계
...
//      set<필드명>();
//      setName(String name), setAge(int age), getStudentId(int studentId)
...
class Student {
    ...
    // cf) getter 는 단순한 데이터의 반환 뿐만 아니라, 데이터 가공 역할을 담당하기도 함
    ...
    //cf) setter 는 단숭ㄴ한 데이터 설정 뿐만 아니라, 데이터의 검증 역할을 담당하기도 함
...
public class B_Encapsulation {
    public static void main(String[] args) {
        ...
        //student.name - private 설정(해당 클래스 외어ㅔ서는 접근 불가)
        ...
        student.setScore(108);
        student.setScore(100);

        student.setResidentNumber(987456);
