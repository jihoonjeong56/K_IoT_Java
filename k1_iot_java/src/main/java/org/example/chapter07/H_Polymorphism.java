...
// ==다형성(Polymorphism)
// : 많으 ㄴ형태를 가질 수 있는 특성
// - 하나의 객체가 여러 타입의 인스턴스(객체)로 표현될 수있는 특성
...
class Bus extends Vehicle {
    @Override
    void depart() {
        System.out.println("버스가 출발합니다. 자리에 않아주세요");
    }
}

class Subway extends Vehicle {
    public Subway() {
        System.out.println("문이 닫힙니다. 열차가 출발 합니다.");
    }
}
class Bird{
    void flay(){
...
public class H_Polymorphism {
    public static void main(String[] args) {
        ...
        if(subway instanceof Vehicle){
            //subway 객체가 Vehicle 인스턴스 || Vehicle 을 상속받은 인스터스일 경우
            subway.depart();
        }
        ...
        if (vehicle instanceof Bus) {
            System.out.println("운송수단은 버스 입니다.");
        }  else {
