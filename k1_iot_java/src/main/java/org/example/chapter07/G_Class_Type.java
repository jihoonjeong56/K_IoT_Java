...
class G_Child extends G_Parent{
    ...
    void childMethod(){
        System.out.println("자식 클래스만이 가지는 메서드 입니다.");
    }
}
...
public class G_Class_Type {
    ...
    public static void main(String[] args) {
        ...
        //cf) 자동 형 변환(업캐스팅)의 경우 해당 객체는 보존되면서, 부모 캑체처럼 사용
        ...
        // 자식타입 객체명수명 = (자식타입) 부모객체
        G_Child g_child = (G_Child) parentObject;
        g_child.displayInfo();
        g_child.childMethod();

        // cf) 클래스 타입 변환 시 주의점
        // : 반드시 상송 관계와 객체 실제 타입을 확인
