...
/*
    ===스트림(Stream, 흐름) API===
    : 자바 8부터 도입된 기능, 컬렉션 또는 배열에 저장된 데이터를 선언형(직관적, 간결함) 방식으로 처리할 수 있게 지원하는 도구
    - 데이터 필터링, 변환, 매핑, 집계 등의 다양한 작업 수행
    - 람다식과 결합하여 가독성 높은 코드 작성 가능

    1. 스트림의 3단계 구조(데이터 파이프라인)
    : 데이터를 소스로 부터 연속적으로 처리하는 '파이프라인' 구축
    ...
    - 스트림으로 결과가 반환되기 때문에 체이닝이 가능
    - 지연처리: 최종 연산이 실행되기 전까지 동작하지 않음
    ...
    2. Stream API 특징
    1) 불변성 : 원본 데이터를 변경하지 않는다, 새로운 값 반환
    2) 지연처리 : 중간 연산은 실제로 실행되지 않음, 최종 연산 시 처리됨
...
public class H_Stream {
    public static void main(String[] args) {
        ...
        //3. 최종 연산
        // : 스트림 소모 + 결과 생성
        // - forEach: 각 요소에 대해 동작 수행(반환값은 없음)
        // - collect : 컬렉션으로 반환
        ...
        System.out.println("이름에 \"정\" 이름이 붇은 사람 뒤에 \"님\" 자를 붙여서 출력");
        streamNames.filter(name -> name.startsWith("정"))
                .map(name -> name + "님")
                //.forEach(name -> System.out.println(name));
                .forEach(System.out::println);
        //2)
        System.out.println("이름의 길이가 3글자인 사람만 출력");

        names.stream()
                .filter(name -> name.length() == 3)
                .forEach(System.out::println);
        //3)
        System.out.println("중복 제거후 오름차순 정렬하여 출력");
        names.stream()
                .distinct()
                .sorted()
                .forEach(System.out::println);
        //4)
        System.out.println("총인원수 출력");
        long count = names.stream().count();
        System.out.println("총인원수: " + count);
        ...
        System.out.println("합친 이름들: " + result2);
        ...
        names.stream()
                ...
                // >> b가 더 크면 양수
                // >> 더큰 값을 앞으로 정렬(양수면 b 가먼저, 음수면 a가 먼저, 0이면 그대로)
