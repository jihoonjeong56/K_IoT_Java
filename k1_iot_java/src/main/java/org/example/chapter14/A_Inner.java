 ...
 *   >> 외뷔 클래스의 인스턴스 멤버(필드, 메서드)에 접근 가능
 * 2. 정적(static) 내부 클래스
 *   : "외부 클래스의 정적멤버" 에 속함
 ...
 *      >> 해당 메서드 내에서만 사용 가능
 *      >> 메서드의 지역 변수에만 접근 가능
 *      cf) 로컬변수(외부클래스의 멤버 변수)가 final인 경우에만 접근 가능
 * 4. 익명 (내부) 클래스
 *  : 이름이 없는 내부 클래스
 *      - 주로 인터페이스나 추상 클래스의 구현에 사용된다.
 *      >> 즉시 객체 생성 + 일회성 사용
 * */

class OuterClass {
    private String outerField = "외부클래스의 인스턴스 필드";
    ...
    // 3. 메서드 내부 클래스(지역클래스)
    void outerMethod(){
        // 지역변수는 암묵적으로 final로 간주(생략 가능)
        //      >>  재할당이 이루어지면 final 이 사라짐(final로 취급 x)
        ...
        class MethodClass{
            void display()  {
                System.out.println("로컬변수에 접근(final X: )"+ localVar); // 유사 final
                System.out.println("로컬변수에 접근(final o: )"+ localFinalVar); // final
...
public class A_Inner {
    public static void main(String[] args) {
        ...
        //2)외부 클래스의 인스턴스처럼 사용: 외부클래스객체명.클래스 호출
        ...
        System.out.println("====메서드 내부 클래스(지역클래스)====");
        ...
        AbstractClass abstractClass2 = new AbstractClass() {
            @Override
            void display() {
                System.out.println("추상 클래스 2");
            }
        };
        abstractClass2.display();
        System.out.println(abstractClass1.equals(abstractClass2)); // false

        //클래스 정의가 외부에서 재사용 될 필요가 없거나, 단 한번의 사용 목적에 유용
        InterfaceClass interfaceClass = new InterfaceClass() {
            @Override
            public void something() {
                System.out.println("안녕하세요 인터페이스로 구현한 익명 클래스 입니다.");
            }
        };
