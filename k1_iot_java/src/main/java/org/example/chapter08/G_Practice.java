...
interface TropicalFruit extends Fruit {
    ...
    default void tropicalFeature() {
        System.out.println("열대과일은 훅속하면 더 맛있습니다.");
    }
}
...
public class G_Practice {
    public static void main(String[] args) {
        ...
        //==업캐스팅
        //:mango를 Fruit 타입으로 형 변화
        // > TropicalFruit가  Fruit을 상속 받는 인터페이스
        Fruit[] fruits = {apple, banana, orange, mango};
        for(Fruit fruit : fruits){
            fruit.describe();
            //fruit.tropicalFeature();
            if(fruit instanceof TropicalFruit){
                //다운캐스팅 - 해당 클래스만이 가지느 고유 메서드에 접근
       ...
       class 동물{}
        ...
        if(나비 instanceof 동물){
            System.out.println("실행됨 나비는 동물의 일종");
        }
        if(나비 instanceof 고양이){
            System.out.println("실행됨 나비는 원래 고양이 고양이 만이 가진 특정 기능을 수행하기를 원함");
            고양이 야옹  = (고양이) 나비; //if 조건문 외부에서도 가능 정상코드 !  다만, 확실하게 검증하고 변환하자
        }
