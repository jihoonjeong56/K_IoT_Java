...
// 4. 사용 목적
// : 확장성 제공을 위해 사용
//      - 자식 클래스가 공통된 기능을 공유 + 공유 기능을 구현하도록 강제
...
public class A_Abstract {
    public static void main(String[] args) {
        ...
        cat.makeSound();
        cat.eat(); //- 부모의 메서드 결과가 호출

        //추상 클래스 또한클래스 타입으로 활용 가능;
        // >> 단, 자식만이 가진 멤버(필드 &메서드)에 접거ㅡㄴㅂ ㅜㄹ가
        Animal animalDog = new Dog();
        animalDog.makeSound();
        animalDog.eat(); // - 오버라이딩 된 결과가 출력
        //animalDog.bark();'Animal'의 메서드 'bark'을(를) 해결할 수 없습니다.
