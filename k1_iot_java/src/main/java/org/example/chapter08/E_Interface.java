...
// cf) 클래스||인터페이스 '맴버': 해당 구조의 내부' 구성요소'
...
class ExampleClass implements Example1, Example2 {
    ...
    public void printVar2() {
        // "해당 메서드의 추상 메서드 클래스(Example2)" 에서 상수 값이 없더라도 사용 가능
        System.out.println(EXAMPLE_VARIABLE);
    }
    ...
    //다중인서페이스 구현 시 같은 시그니처를 가진 디폴트 므ㅔ서드가 존재할 경우
    //      -반드시 오버라이드 어노테이션을 작성
    @Override
    public void method() {
...
public class E_Interface {
    public static void main(String[] args) {
        ...
        example1.printVar1();
        //example1.printVar2(); - Example1에 정의 되지 않음
        example1.method();
