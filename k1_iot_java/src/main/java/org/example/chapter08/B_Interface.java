...
// cf) >> (프로그램 전반의) 규칙(규약): public static final 특성을 가짐
...
// 3. 인터페이스 구현 방법
// : interface 키워드를 사용 (인터페이스명은 클래스 명 지정과 동일: UpperCamelCase)
// - interface 인터페이스명 {}
...
interface PlayingCard {
    ...
    // void defaultMethod2(){
    //     System.out.println("default 생략 불가.");
...
class Card implements PlayingCard {
    ...
    private String cardKing;

    Card(String cardNumber, String cardKing) {
...
public class B_Interface {
    public static void main(String[] args) {
        ...
        System.out.println(card.getCardKing());
        System.out.println(card.getCardNumber());

        card.defaultMethod(); //Card 클래스에서 재정의 된 디폴드 메서드
