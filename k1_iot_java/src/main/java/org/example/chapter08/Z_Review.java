...
abstract class TransportApp{
    abstract void move();
    void info(){
        System.out.println("이동관련 애플리케이션 입니다.");
    }
...
interface EcoFriendly {
    default void ecoMessage(){
        System.out.println("이동수단은 친환경적 입니다.");
    }
}
class Move1 extends TransportApp{
    @Override
    void move() {
...
public class Z_Review {
    public static void main(String[] args) {
        TransportApp transportApp = new Move1();
        transportApp.move();

        TransportApp[] transportApps = {
          new TaxiApp(), new BikeApp()
        };
        for(TransportApp transportApp1 : transportApps){
            transportApp1.move();
            transportApp1.info();

            if(transportApp1 instanceof EcoFriendly){
                EcoFriendly ecoFriendly = (EcoFriendly) transportApp1;
