...
//      - 자식 클래스의 생성자 호출 시 '반드시' 부모 클래스의 생성자가 호출(사용자 정의일 필요 없음)
//      - 반드시 부모가 존재해야만 자식이 존재
class A {
    A() {
    }// 빈생성자 - 사용자 정의 생성자가 없으면 기본 생략
...
class Cat extends Mammal {
    ...
    //cf)상속받은 자식 클래스의 경우 : 사용자 정의 부모 생성자의 호출이 없더라도  super() 부모 클래스 생성자 호출 키워드가 정의되어 있음(생략)
    Cat() {
        super(); //생략되었어도 자동 호출
        // >> Mammal 클래스 내애 매개변수가 없는 생성자.
    ...
    Cat(String name) {
        ...
        // - 부모 클래스 내애 정의된 생성자를 형태 그대로 사용
        // - 여러 개의 생성자가 있는경우(오버로딩) super()의 형태도 다양할 수 있음
        //this.name = name;
    }

    void displayCat() {
        //해당클래스가 가진 인스턴스 변수에 접근: this >>생략가능(지역변수와 충돌이 나지 않는 경우)
        System.out.println("자식 이름: " + name);
        System.out.println("자식이름: " + this.name);

        //부모 클래스가 가진 인스턴스 변수에 접근: super
        // >> 생략 불가 - 생략시 자식에 대한 변수로 착각
