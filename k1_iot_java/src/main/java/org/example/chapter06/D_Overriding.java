...
//===오버라이딩(Overriding)===
// : 자식클래스가 부모 클래스에서 상속받은 '메서드'를 자식의 상황에 맞게 '재정의'
...
//    }
    ...
    // : 자바 컴파일러가 메서드 시그니처(반환타입, 이름, 매개변수)를 비교하여, 부모클래스의 메서드에서 재정의했는지 판다.
    void exercise() {
...
//===오버라이딩의 틍징
//1. 메서드 시그니처가 일치
// : 부모 클래스의 메서드와 동일한 이름, 매개변수 구성을 가져야 한다.
//     + 반환 타입은 일치하거나 자동 형 변환이 가능해야 한다.
...
class DogClass extends AnimalClass{
    @Override
    void sound(){
        System.out.println("강아지가 멍멍 하고 짖습니다.");
    }
}
...
public class D_Overriding {
    public static void main(String[] args) {
        ...
        dogClass.sound(); // 각각의 재정의된 기능 수행
        catClass.sound();
        // >> 해당 상태나 동작을 반드시 가지지만
        //      , 상속받는 클래스마다 차이를 두고싶은경우 - 오버라이딩 활용
        ...
        // 2) 오버라이딩
        // - "상속관계"에 있는 클래스에서 사용
        // : 메서드 시그티처(반환타입, 이름, 매개변수)가 동일
        // : 메서드의 동작을 변경하거나 확장
        // +) @Override 이노테이션 사용 : 컴파일러에게 전달
