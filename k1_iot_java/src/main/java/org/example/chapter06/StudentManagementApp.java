...
public class StudentManagementApp {
    ...
    //      :하나의 파일에서 public 단위의 클래스느 한 개만 지정 가능
    //      >> public class 클래스 명은 파일명과 일치
    ...
    static void addStudent(int id, String name, int age, String major, double gpa) {
        ...
        students.add(student); // .add(리스트의 요소타입과 일치한 데이터만 삽입가능);
    ...
    static void printAllStudents() {
        ...
        //for(순회하는 요소의 연수 선언: 배열리스트
        for (Student student : students) {
            student.printStudentInfo();
        }
    }

    //학생 검색: 학생 Id를 통해 학생의 정보 출력
    static void findStudentById(int studentId) {
        ...
        System.out.println(studentId + "에 해당 ID의 학생을 찾을 수 없습니다.");
    }
    ...
    // > JVM은 main 메서드가 없을 경우 해당 파일을 실행 할 수 없음
