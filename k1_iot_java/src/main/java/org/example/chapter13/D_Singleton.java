...
// 4) 구현 방법
class Singleton {
    // 필드값: static 으로 - 인스턴스화 없이 호출 가능 & 자원이 하나의 값을 가짐
    private static Singleton instance;

    //생성자 : private >> 외부에서 직접적인 인스턴스화 불가
    ...
    public static Singleton getInstance() {
        // 주로 getInstance 메서드명 사용 : 반환 타입은 클래스 타입
        if (instance == null) {
            //필드에 초기화 이전 >> 아직 인스턴스화 안함
...
class SchoolPrincipal{
    ...
    //cf) 생성자: 반환 타입 없음, 접근제한자 가능 , 이름은 클래스 명과 동일
    ...
    public static SchoolPrincipal getInstance(){
        ...
        return instance; // 기존 객체(if가 false) || 생성 객체(if가 true) 반환
    }
    public void announce(){
        System.out.println("공부 열심히 하고 운동 열심히 하고 일찍 일어나고");
    }
}
class Example{
}
