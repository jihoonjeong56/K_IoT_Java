 ...
 * - Optional<T>: 값이 있을 수도 없을 수 도 있는 객체 래퍼
 *
 * */
...
public class B_Generic {
    public static void main(String[] args) {
        ...
        Message<?> m1;
        Message<String> m2;
        //형변화: Object 클래스는 모든 참조 타입의 최상위 클래스
        //      - 모든 타입 클래스와 상속 관계
        ...
        //list1.add("안녕");//필요한 타입:capture of ?, 제공된 타입:String - 타입의 불분명성으로 타입 추가가 불가
        ...
        //  : 이 리스트는 Integer의 상위 타입중 하나로 선언된 리스트
        //  >> 컴파일러 입장에서는 해당 타입의 객체가
        //  List<Integer> List<Number> List<Object> 인지 알 수 없음
        //+) Double 이 Integer의 하위타입 아님 >> 형제 관계

        // Integer와 호환되는 값: Integer, int, null;
    ...
    // : Number의 자손 타입중 하나이기 하나, 정확히 어떤 타입인지 확인 불가
    // >> 읽기는 가능, 쓰기는 제한
    Number num = numberList.get(0); //읽기는 가능
    //numberList.add(10);// 컴파일 에러발생 - 구체적인 숫자값 추론 불가(타입 안정성 보장 불가)
