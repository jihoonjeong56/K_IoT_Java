...
// - 어노테이션(코드주석)을 사용하여 코드를 간결하게 만들어 주는 역할
...
// +) Maven Repository
//: Java 프로젝트에 필요한 라이브러리(외부코드)들을 쉽게 다운로드하고 관리할 수 있도록 도와주는 저장소
...
// 1. @Getter/Setter : 필드에 대한 getter 와 setter 메서드를 자동 생성

// 2. @ToString : 클래스의 toString() 메서드를 자동 구현
// - 객체 출력 시 지정된 형식으로 출력
// - 클래스명(필드명1 = 필드값1,........)
...
// cf) @EqualsAndHashCode: 두 객체의 내용이 같은지(동등성) 비교 & 두 객체가 같은 객체인지(동일성) 비교 연산자
// >>  boolean 값 변환

// 5. @Builder 어노테잇견
// : 복잡한 객체 생성시 사용하는 빌더 패턴을 자동으로 구현
// - 필드값을 모두 메서드 체이닝으로 가져옴(필수값 또한 메서드 체이닝으로 구현)
// - 메서드 체이닝 이름은 필드명
// >> 필수값을 체이닝에서 누락 시 오류
@Getter
@Setter
...
class LombokClass{
    ...
    @NonNull // null 값이 들어갈 수 없음(필드에 필수값 지정): 값의변화는 가능(final 불변성과의 차이)
...
public class G_Lombok {
    public static void main(String[] args) {
        LombokClass lombokClass1 = new LombokClass("자바는 재밌어","123","ㅁㄴㅇ");
        LombokClass lombokClass2 = new LombokClass("java","asdf");
        LombokClass lombokClass3 = new LombokClass.LombokClassBuilder()
                // @Builder 에서 필수 체크값 아님
                // : Lombok이 내부에서 빌더 전용 생성자를 생성 초기화를 우회(예외)
                .title("자바")
                //@Builder:  @NonNull 이 있는 필드에 대해 값을 할당 하지 않으면 NullPointerException이 발생
                .author("ㅁㄴㅇㄹ")
                .build();
