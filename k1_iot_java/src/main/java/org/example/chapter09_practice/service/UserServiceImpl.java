...
public class UserServiceImpl implements UserService {
    ...
    // C(Create) : 데이터 생성(회원가입)
    // R(Read)  : 데이터 조외( 게시글 목록 보기)
    // U(Update) : 데이터 수정(내 정보 수정)
    // D(Delete) : 데이터 삭제(게시글 삭제)
    ...
    public User getUserById(int id) {
        ...
        for(User user: userList){
            ...
            // : for 문 내에서 return 키워드를 만나지 않고 전체 순회가 끝나면 예외 발생
            if(user.getId() == id){
                return user;
            }
        }
        throw new UserNotFoundException("사용자 ID에 "+ id+ "값이 없습니다.(조회실패)");
    }
    ...
    public void deleteUser(int id) {
        ...
        if(userToDelete == null){
            ...
            throw new UserNotFoundException("사용자 ID에 "+ id+ "값이 없습니다. (삭제실패)");
        }
        //삭제 가능
    ...
    public List<User> findUsersByName(String name) {
        ...
        for(User user: userList){
            //전체 사용자를 순회하며 매개변수의 이름값과 일치하는 경우 result 에 삽입
            if(user.getName().equals(name)){
                //문자열.equals(문자열) - 결과값을 boolean으로 반환
                result.add(user);
            }
        }
