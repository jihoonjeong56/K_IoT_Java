...
class MyClass extends Object {
    ...
    public boolean equals(Object obj) {
        // 현재 this 와 매개변수의 Object가 동등한지 비교
        // - 두 객체의 참조를 비교(주소값)
...
public class JavaLang {
    public static void main(String[] args) {
        ...
        MyClass myClass1 = new MyClass(1, "asdf");
        System.out.println(myClass1); //MyClass[ID: 1 / NAME: asdf ]
        System.out.println(myClass1.toString()); //MyClass[ID: 1 / NAME: asdf ]
        System.out.println(myClass1.displayInfo()); //MyClass[ID: 1 / NAME: asdf ]

        MyClass myClass2 = new MyClass(2, "wasd");
        //equals()베서드
        // : 인스턴스 메서드
        ...
        // : 문자열의 경우 equals() 메서드가 두 문자열 내용(값)을 비교

        System.out.println("==Math==");
        // Math 클래스 내부의 모든 필드와 메서드는 static

        System.out.println(Math.abs(-10)); //absolute value: 절대값
        System.out.println(Math.max(10, 20));// 두 가지중 최대값
        System.out.println(Math.min(10, 20));// 두 가지중 최솟값
        System.out.println(Math.sqrt(16)); //  square root : 제곱근
        System.out.println(Math.pow(2,3)); // pow(a,b) : a의  b 제곱
        System.out.println(Math.random()); // 무작위 난수를 생성 : 0.0 이상 0.1 미만의 난수를 생성

        System.out.println("==String==");
        String message = "Merry Christmas!";

        System.out.println(message.length()); // 문자열의 길이 반환(공백, 기호도 반환)
        ...
        System.out.println(message); //문자열 기능을 활용해도 기본 데이터 변환 x >> 문자열 불변성
