...
/*
    ...
    1. throw
     ...
     > 주로 조건무과 함께 사용(프로그램의 흐름을 제어하는 역할)

     [사용방법]
     : throw new ExceptionType("예외메시지");
     - 인자로 전달하는 문자열은
     - 예외변수(e).getMessage() 메서드를 활용하여 catch 블록에서 출력가능

    2.throws
     : 메서드 선언부에 위치, 호출자에게 예외 처리를 위임
     - 체크(컴파일) 예외: 필수 명시 / 언체크(런타임) 예외: 선택명시
...
public class Exception03 {
    ...
    static void exceptions(int num) throws CustomException, ArithmeticException, ArrayIndexOutOfBoundsException {
        if (num == 1) {
            ...
            throw new ArithmeticException("2번선택 - 수학 공식 예외");
        } else {
    ...
    public static void main(String[] args) {
        ...
        try {
            if (age < 0) {
                throw new Exception("나이는 음수가 될수 없습니다.");
            }
            System.out.println("예외발생시 실행 안됨/ 예외발생 하지 않으면 실행");
        } catch (Exception e) {
        ...
        //exceptions(1);
        try {
            System.out.println("여러 예외 던지기테스트");
            //exceptions(1);
            //exceptions(2);
            exceptions(3);
