package org.example.z_project.user_reservation.service.impliment;
...
public class UserServiceImpl implements UserService {
    ...
    private User loggedUser;// 로그인된 사용자 정보를 정장하는 필드
    ...
    public void registerUser(UserRegisterRequestDto dto) {
        ...
        // cf) 옵셔널 데이터 .get():Optional 내부의 데이터를 반환
        // - 값이 존재하지 않을때 NuSuchElementException 예외 발생
        Optional<String > optional = Optional.empty();
        //optional.get(); // NuSuchElementException
        //cf) isPresent(), isEmpty()
        if(optionalUser.isPresent()){
            //해당 id의 유저가 존재 - 회원가입 불가
            System.out.println("해당 ID의 유저가 존재합니다 .다시 시도해 주세요");
            return; //메서드 종료
        }
        //회원가입 정상 로직 실행
        ...
        if(!password.equals(passwordCheck)){
            //비밀번호와 그 확인이 일치하지 않으면 회원가입 불가
            System.out.println("비밀번호와 그 확인이 일치하지 않습니다. 다시 시도해 주세요");
            return;
        }
    ...
    public void logout() {
        if(loggedUser != null){
            ...
            System.out.println("로그인 상태가 아닙니다. 로그인하여 이용해주세요");
        }
    }
    ...
    public boolean isLoggedIn() {
        ...
        //로그인 중이면 true, 로그인중이 아니면 false 반환
        return result;
    }
    @Override
