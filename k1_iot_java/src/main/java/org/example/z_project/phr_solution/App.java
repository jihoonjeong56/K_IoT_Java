...
/*
    ...
    1. 프로젝트 개요
     - 개인건강 기록(PHR) 솔루션을 위한 간단한 콘솔 기반 응용 프로그램
     - 환자의 기본 정보를 관리, 환자의 건강 기록 등록, 수정, 조회 가능
     - 특정 조건에 맞는 건강 기록을 필터링 하고 집계 가능
    ...
    3. 비즈니스 로직 구조
     ...
     - DTO : 계층간 데이터 전송을 위한 객체
     - entity : 데이터 저장소(DB) 의 테이블과 매핑(연결) 되는 도메인 객체

    4. 실행 및 관리
     - App.java: 프로그램 실행에 관한 진입점, 사용자와의 상호작용을 관리하고 전체
            >> App.jave가 사용자 입력 처리, 메뉴 훌력, 흐름 제어 모두 담당 -> SRP 위반(단일 책임 원칙)
...
public class App {
    ...
    private static boolean processChoice(int choice) {
        switch (choice) {
            ...
            default:
                System.out.println("잘못된 선택입니다 유효한 메뉴 선택");
                break;
        }
        return true;
