...
/*
    ...
    "좋은 소프으웨어 일수록 모듈의 독립성이 높다."
    >> 모듈의 독립성을 결함도(coupling) 와 응집도(cohesion)의 기준 단계로 측적
...
class ClassRoom{
    private Teacher teacher; //인터페이스 타입으로 선언 - 리스코프 치완 원칙(LSP)

    //의존성 주입(Dependency Injection) - DIP
...
class Attendance {
    ...
    public void checkAttendance(Teacher teacher, StudentCohesion studentCohesion){
        System.out.println(studentCohesion.getName()+" 학생이 출석합니다.");
        teacher.teach();
    }
}
// cf) 의존성 주입(DI Dependency Injection)
// : 객체가 필요한 의존 객체를 직접 생성하지 않고, 외부에서 주입받는것
// - 결합도 앉아짐
// - 테스트 용이
// - 코드 재사용성 증가
...
// >> ClassRoom 이 직접 객체 생성하지 않고 , 외부에서 주입 방식(IoC 예시)
