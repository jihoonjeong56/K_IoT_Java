...
// >> 다양한 디자인패턴(Design Pattern) 이 SLOID 설계 원칙에 따라 만들어짐

// cf) SOLID 원칙 적용시 주의점
//      : 적용 순서 없음, 모두 적용할 필요 없음 , 서로 독립적 개념이 아님 - 개념적으로 연관 존재
//      - 상속 , 캡슐화 , 추상화, 다형성은 OOP의 4가지 특성

//      : 소프트웨어 개발 과정에서 자주 발생하는 문제들에 대한 일반적인 해결책
//cf) 디자인 패턴
// 1. SRP(Single Responsibility Principle, 단일 책임 원칙)
// - 클래스는 단 하나의 책임만을 가짐(완전한 캡슐화) / 하나의 변화 이유만을 가져야 함
// - 하나의 클래스가 가지는 하나의 역할과 관련된 변경사항만 클래스에 영향으 미쳐야 한다.
...
// 2) 올바른 설계 예시 - 각각의 클래스가 자신의 역할만 담당(유지보수와 테스트가 쉬워짐)
class Janitor{
    void cleanClassroom(){
        System.out.println("교실을 청소합니다.");
    }
...
// : 소프트웨어 엔티티(클래스, 모듈, 함수 등)는 확장에는 Open 해야하고, 수정에는 Close 해야 한다.
// 1) 잘못된 설계 예시 - 새로운 학생 유형이 생길 때만다 if문 추가(기존코드 수정 필요)
class LunchMenu{
...
class  RegularStudent implements LunchMenuInterface{
    ...
    public void serveLunch() {
        System.out.println("일반학생 : 알레르기가 없습니다.");
    }
}
...
// 3. LSP( Liskov Substitution Principle, 리스코프 치환(교체) 원칙)
// - 상위클래스의 객체를 하위 클래스의 객체로 치환하더라고 프로그램의 동작이 일관되게 유지 되어야 함.(상위클래스의 행동 규약을 하위클래스가 위반하면 안된다).
// - 자식클래스는 부모 클래스의 기능을 완전하게 대체할 수 있음
// >> 업캐스팅 상태에서 부모의 메서드 사용시 동작 가능
// 1) 잘못된 설계 예시 - Student의 동작과 InjuredStudent 가 같은 메서드를 통해 서로 다른 행동을 실행함
class Student{
    void playSoccer(){
...
// 2) 올바른 설계 예시 - 부모의 activity() 라는 추상 메서드르 각 자식의 의도에 맞게 재정의
abstract class StudentAbstract{
    abstract void activity();
}
class SoccerPlay extends StudentAbstract{
    @Override
    void activity() {
