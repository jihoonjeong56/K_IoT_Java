...
public class LibraryManager implements Manageable {
    ...
    public List<Item> search(String keyword) {
        // 전체 리스트의 요소에서 키워드 검색
        // : name(책 이름), author(저자명), publisher(출판사)

        List<Item> foundItems = new ArrayList<>();// 검색된 도서 정보들이 저장될 배열(필터링 된 요소들)
    ...
    public List<Item> searchByCategory(String category) {
        ...
        for (Item item : items) {
            if (item instanceof Book && ((Book) item).getCategory().equalsIgnoreCase(category)) { // equalsIgnoreCase : 대소문자를 구분하지 않고 데이터의 값을 비교 (영어만 사용 가능)
                result.add(item);
            }
        }
        if (result.isEmpty()) {
