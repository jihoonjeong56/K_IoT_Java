...
// 1. 특징
// : 순서 유지(인텍스를 통해 정확한 위치 지정 가능), 중복 저장 가능
...
//      - 추가/제거: 공간에 대한 정리(중간 삽입 시 나머지 요소를 한 칸씩 이동) >> 시간 복잡도 발생 O(n)
//      - 꺼내기(조회): 순차적인 수를 사용하여 꺼냄(메모리 상의 위치로 바로 이동 가능) >> 속도빠름
...
// EX) '기차' - 칸마다 서로 연결 정리
//       - 새로운 칸(객차)를 추가/삭제 - 앞뒤 칸에 연결만 하면 가능 >> 속도 빠름
//       - 특정 칸 찾기(조회): 앞에서 부터 순차 탐색 >> 속도 느림
...
public class B_List {
    public static void main(String[] args) {
        ...
        //List 인터페이스 >> AbstractList 추상 클래스 >> ArrayList 클래스 & LinkedList 클래그
        ...
        System.out.println(removeElement + "가 삭제 되었습니다.");
        System.out.println(arrayList);
        //LinkedList & ArrayList 동일
        ...
        //      : 클래스 구조는 객체 타입의 데이터만 삽입 가능(Integer, Character, Boolean, Double 등등등등....)

        long startTime = System.nanoTime();
        //1초 = 1,000,000,000 나노초
        System.out.println(startTime);
        for (int i = 0; i < 2222; i++) {
            arrayListTest.add(0, i);//항상 맨 앞 요소 삽임
        }
