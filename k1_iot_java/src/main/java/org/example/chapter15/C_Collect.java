...
public class C_Collect {
    public static void main(String[] args) {
        ...
        //Collectors.joning(delimiter-구분자, prefix -접두사, suffix-접미사)
        ...
        // : 조건에 따라 두 그룹으노 나눔(해당 조건이 참/거짓인 경우로 나뉨)
        Map<Boolean, List<Integer>> partitioned = Stream.of(1, 2, 3, 4, 5)
                .collect(Collectors.partitioningBy(num -> num % 2 == 0));
        System.out.println(partitioned);//{false=[1, 3, 5], true=[2, 4]}

        Map<Boolean, List<String>> partitioned1 = languages.stream()
                .collect(Collectors.partitioningBy(string -> string.contains("java")));
