...
/*
    1. 불변성 데이터: Stream API 정렬 사용
     목적) 원본리스트 변경 안하고, 정렬된 새로운 리스트만 필요시
    ...
    ex) 캐시된 리스트, 재사용 리스트, 통계 계산용 리스트 등.............
        >> 데이터베이스나 외부 API 에서 자주 조회되는 데이터를 미리 리스트에 저장해두고, 필요 시 다시 원본을 가져오지 않고 빠르게 읽어오는 용도
...
class ProductService {
    ...
    //수정
    void updateProduct() {
        //가격 기준으로 정렬된 새로운 리스트 반환(원본  product 는 변경되지 않음
    }
...
class PostService {
    ...
    //stream()보다 메모리 효율이 좋음(새 리스트 생성 안함)
    // : 캐시나 반복 사용 데이터에 적합
...
class UserComparators {
    ...
    //정렬로직을 메서드에서 분리하여 한 곳에서 관리 가능
    // : 테스트 용이성, 유지보수성 향상
    // - 실부 프로젝트에서는 ComparatorUtils, SortConstants 클래스에서 모아서 정의
}
...
//SQL 정렬 VS 자바의 정렬의 역할 분리
// >>  성능 최적화 및 역할 구분
// 1) 데이터가 많고 단순한 정렬 : SQL - ORDER BY
// 2) 복잡한 정렬 기준 : Java - Comparator, Stream API
// 3) 정렬 기준이 요청마다 달라짐 : 자바 처리
// 4) 정렬된 순서를 DB에서 본다: SQL 처리